...
class BinaryReader {
  ...
  seek(offset: number): void {
    if (offset < 0 || offset > this.buf.length) {
      throw new Error(
        `Seek out of bounds: ${offset} (file size: ${this.buf.length})`,
      );
    }
    this.pos = offset;
  }
  ...
  u8(): number {
    if (!this.hasBytes(1))
      throw new Error(`Read u8 past end at offset ${this.pos}`);
    return this.buf[this.pos++];
  }

  u16(): number {
    if (!this.hasBytes(2))
      throw new Error(`Read u16 past end at offset ${this.pos}`);
  ...
  u32(): number {
    if (!this.hasBytes(4))
      throw new Error(`Read u32 past end at offset ${this.pos}`);
  ...
  u64(): bigint {
    if (!this.hasBytes(8))
      throw new Error(`Read u64 past end at offset ${this.pos}`);
  ...
  bytes(n: number): Buffer {
    if (!this.hasBytes(n))
      throw new Error(`Read ${n} bytes past end at offset ${this.pos}`);
...
function parseCoffHeader(r: BinaryReader, peOffset: number): CoffHeader {
  ...
  return {
    machine,
    machineDescription:
      MACHINE_TYPES[machine] || `Unknown (0x${machine.toString(16)})`,
    numberOfSections,
    timeDateStamp,
    timeDateStampDate:
      timeDateStamp > 0
        ? new Date(timeDateStamp * 1000)
            .toISOString()
            .replace("T", " ")
            .replace(/\.\d+Z$/, " UTC")
        : "N/A",
...
function parseOptionalHeader(
  ...
  return {
    ...
    subsystemDescription:
      SUBSYSTEM_TYPES[subsystem] || `Unknown (${subsystem})`,
    dllCharacteristics,
    dllCharacteristicFlags: decodeFlags(
      dllCharacteristics,
      DLL_CHARACTERISTICS,
    ),
    numberOfRvaAndSizes,
...
function parseSectionHeaders(
  ...
  for (let i = 0; i < count; i++) {
    ...
    sections.push({
      ...
      numberOfRelocations,
      characteristics,
      characteristicFlags: decodeFlags(
        characteristics,
...
// ─── Runtime Dependency Scanner ────────────────────────────────────────────

/** Scan binary for DLL names loaded dynamically via LoadLibrary (not in import tables) */
function scanRuntimeDependencies(
  buf: Buffer,
  ownName: string,
  importedNames: Set<string>,
): string[] {
  const dllPattern = /^[A-Za-z0-9][A-Za-z0-9_.\-]{1,80}\.dll$/i;
  const found = new Set<string>();

  // Scan for ASCII null-terminated strings ending in .dll
  let i = 0;
  while (i < buf.length - 5) {
    // Quick check for '.dll' or '.DLL' at ASCII level
    if (
      buf[i] === 0x2e /* . */ &&
      (buf[i + 1] === 0x64 || buf[i + 1] === 0x44) /* d/D */ &&
      (buf[i + 2] === 0x6c || buf[i + 2] === 0x4c) /* l/L */ &&
      (buf[i + 3] === 0x6c || buf[i + 3] === 0x4c) /* l/L */
    ) {
      // Walk backward to find start of string
      let start = i - 1;
      while (start >= 0 && buf[start] >= 0x20 && buf[start] < 0x7f) {
        start--;
      }
      start++;
      const candidate = buf.toString("ascii", start, i + 4);
      if (dllPattern.test(candidate)) {
        found.add(candidate);
      }
    }
    i++;
  ...
  // Scan for UTF-16LE strings ending in .dll (Delphi UnicodeString, .NET, etc.)
  i = 0;
  while (i < buf.length - 10) {
    // Quick check for '.dll' in UTF-16LE: 2E 00 64/44 00 6C/4C 00 6C/4C 00
    if (
      buf[i] === 0x2e &&
      buf[i + 1] === 0x00 &&
      (buf[i + 2] === 0x64 || buf[i + 2] === 0x44) &&
      buf[i + 3] === 0x00 &&
      (buf[i + 4] === 0x6c || buf[i + 4] === 0x4c) &&
      buf[i + 5] === 0x00 &&
      (buf[i + 6] === 0x6c || buf[i + 6] === 0x4c) &&
      buf[i + 7] === 0x00
    ) {
      // Walk backward to find start of UTF-16LE string (each char is 2 bytes, high byte 0x00 for ASCII range)
      let start = i - 2;
      while (
        start >= 0 &&
        buf[start + 1] === 0x00 &&
      ...
      start += 2;
      const strBuf = buf.subarray(start, i + 8);
      const candidate = strBuf.toString("utf16le");
      if (dllPattern.test(candidate)) {
        found.add(candidate);
      }
    }
  ...
  // Filter out own name and anything already in import/delay-import tables
  const ownLower = ownName.toLowerCase();
  const result: string[] = [];
  for (const name of found) {
    const lower = name.toLowerCase();
    if (lower === ownLower) continue;
    if (importedNames.has(lower)) continue;
    result.push(name);
  }
...
export function parsePeFile(filePath: string): PeFile {
  ...
  const optionalHeader = parseOptionalHeader(
    r,
    optHeaderOffset,
  ...
  try {
    if (optionalHeader.dataDirectories.length > 13) {
      delayImports = parseDelayImports(
        r,
        optionalHeader.dataDirectories[13],
        sections,
        optionalHeader.isPE32Plus,
      );
    }
  ...
  // Runtime dependencies (dynamically loaded DLLs found as strings in the binary)
  const importedNameSet = new Set<string>();
  for (const imp of imports) {
    if (imp.dllName) importedNameSet.add(imp.dllName.toLowerCase());
  }
  for (const imp of delayImports) {
    if (imp.dllName) importedNameSet.add(imp.dllName.toLowerCase());
  }
  let runtimeDependencies: string[] = [];
  try {
    runtimeDependencies = scanRuntimeDependencies(
      buf,
      fileName,
      importedNameSet,
    );
  } catch (e: any) {
    errors.push({ stage: "runtimeDependencies", message: e.message });
  }
...
export function parseImportNames(filePath: string): string[] {
  try {
    ...
    if (optionalHeader.dataDirectories.length > 13) {
      const delayImps = parseDelayImports(
        r,
        optionalHeader.dataDirectories[13],
        sections,
        optionalHeader.isPE32Plus,
